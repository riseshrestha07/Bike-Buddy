...
const CustomerProfile = () => {
    ...
    const fetchUser = async () => {
        try {
            ...
            setFrontPreview(data.user.licenseFrontUrl || "");
            setBackPreview(data.user.licenseBackUrl || "");
            setLicenseNumber(data.user.licenseNumber || "");
            setLicenseExpiry(
                data.user.licenseExpiry
                    ? new Date(data.user.licenseExpiry).toISOString().split("T")[0]
                    : ""
            );
    ...
    const onImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
     ...
     const onFrontLicenseChange = (e: React.ChangeEvent<HTMLInputElement>) => {
     const file = e.target.files?.[0];
     ...
     const onBackLicenseChange = (e: React.ChangeEvent<HTMLInputElement>) => {
     const file = e.target.files?.[0];
    ...
    if (!frontLicense || !backLicense) {
        toast.error("Please upload both sides of your licence.");
        return;
    }
    ...
    try {
        ...
        formData.append("images", frontLicense);
        formData.append("images", backLicense);
        ...
        await axios.put(`/api/auth/edit-profile/${userId}`, {

            licenseFrontUrl: urls[0],

            licenseBackUrl: urls[1],
        ...
        toast.error("Failed to upload licence.");
    ...
    return (
        <section className="p-3 md:p-5 mx-auto md:max-w-7xl">
            ...
            <div className="bg-[#d3ecdc] flex flex-col md:flex-row md:justify-evenly gap-4 justify-center items-center rounded-xl shadow-lg py-7 px-4">
                ...
                <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-6 mt-6">
